...
export default function Carparks({setFocused, focused})
{
    ...
    /**
     * This function is for toggle pinned Carparks
     * @param {*} carparkId 
     */
  ...
  // Create new list of carparks with distance
  ...
  const sortedCarparks = [...carparkWithDistance].sort((a,b) => (
    ...
    /**
     * If carpark was clicked
     */
    if(focused)
    {

        const focusedCarpark = carparkWithDistance.find((carpark) => carpark.id === focused.id) || focused

        return (
            <li>
                <h2>{focusedCarpark.name}</h2>
                <p>{focusedCarpark.availableSpaces} available spaces</p>
                {focusedCarpark.distance != null &&    <p>{focused.distance.toFixed(1)}km away</p>}
            </li>
        )
    }
        ...
        <ul className="list">
            {
                sortedCarparks.map((carpark) => (
                    ...
                    >
                        ...
                        <p><strong>Location: </strong>{carpark.location}</p>
                        {
                            carpark.distance != null && <p>{carpark.distance.toFixed(1)} km away</p>
                        }
                        <button
                            onClick={(e) => {
                                e.stopPropagation()

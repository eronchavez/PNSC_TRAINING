...
export default function Carparks({ focused, setFocused }) {
  ...
  useEffect(() => {
    fetch(`${API}/carparks.json`).then((response) =>
      response
        .json()
        .then((data) => {
  ...
  if (focused) {
    const focusedWithDistance = carparkWithDistance.find((carpark) => carpark.id === focused.id) || focused
    return (
      <ul className="list">
        <li>
          <h3>{focusedWithDistance.name}</h3>
          <p>{focusedWithDistance.availableSpaces} available spaces</p>
          <p>{focusedWithDistance.distance.toFixed(1)} km away</p>
  ...
  return (
    <ul className="list">
      {sortedCarparks.map((carpark) => (
        <li onClick={() => setFocused(carpark)} key={carpark.id}>
          <h3>{carpark.name}</h3>
